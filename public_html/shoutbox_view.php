...
$is_mod = ($userdata['user_level'] == 2) ? true : false;
...
$message = charset_utf_fix( addslashes(htmlspecialchars($_POST['message'])), true );
$sb_user_id = $userdata['user_id'];
$id = intval($_POST['id']);
$id_del = intval($_POST['del']);
$id_edit = intval($_POST['edit_id']);
$id_last = intval($_POST['last']);
$last = ( isset($id_last) && $id_last != '' ) ? $id_last : 0;
$request = ( addslashes(htmlspecialchars($_SERVER['HTTP_SHOUTBOX']) ) == 'shoutbox_js' ) ? true : false;
	...
	if ( $shoutbox_config['shoutbox_on'] && $shoutbox != 'off' && !$shoutbox_banned_view && ( $shoutbox_config['allow_guest'] || $shoutbox_config['allow_guest_view'] || $userdata['session_logged_in'] ) && ($shoutbox_config['allow_users'] || $shoutbox_config['allow_users_view'] || $userdata['user_level'] == ADMIN || $is_mod || $is_jr_admin || $shoutbox_view_group) )
	{
		...
		if ( $nums > 0 )
		{
			...
			while ( $row = $db->sql_fetchrow($result) )
			{
				...
				$name = ( $name_id == ANONYMOUS || $name_id == 0 ) ? $lang['Guest'] : $name;
				$name = charset_utf_fix($name);
				if ( $shoutbox_config['allow_smilies'] && $userdata['show_smiles'] )
				{
					$message = smilies_pass($message);
					$message = preg_replace('/images\/smiles/', 'images/smiles', $message);
				}
