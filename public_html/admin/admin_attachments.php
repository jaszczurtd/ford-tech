...
if ( @$_GET['mode'] == 'cats' || @$_POST['mode'] == 'cats' )
{
	define('MODULE_ID', 56);
}
else if ( @$_GET['mode'] == 'quota' || @$_POST['mode'] == 'quota' )
{
	define('MODULE_ID', 57);
}
else if ( @$_GET['mode'] == 'shadow' || @$_POST['mode'] == 'shadow' )
{
	define('MODULE_ID', 58);
}
else if ( @$_GET['mode'] == 'sync' || @$_POST['mode'] == 'sync' )
{
	define('MODULE_ID', 59);
}
else
...
if( !empty($setmodules) )
{
	...
	$module['Attachments']['Configuration'] = $filename . '?mode=manage';
...
if ($check_upload)
{
	...
	else
	{
		...
		if (!$error)
		{
			...
			if (!$result)
			{
				@ftp_mkdir($conn_id, $attach_config['ftp_path']);
				@ftp_site($conn_id, "CHMOD 0777 " . $attach_config['ftp_path']);
				$result = @ftp_chdir($conn_id, $attach_config['ftp_path']);
			}
			...
			else
			{
				...
				@ftp_mkdir($conn_id, $attach_config['ftp_path'] . '/tmp');
				@ftp_site($conn_id, "CHMOD 0777 " . $attach_config['ftp_path'] . '/tmp');
			}
...
if ($mode == 'manage')
{
	...
	$template->assign_vars(array(
		...
		'L_ATTACHMENT_FTP_PASS' => $lang['Password'],
	...
	else if ( (intval($attach_config['allow_ftp_upload'])) && (intval($attach_config['img_create_thumbnail'])) )
	{
		...
		if (!$error)
		{
			...
			if (!$result)
			{
				...
				@ftp_site($conn_id, "CHMOD 0777 " . $attach_config['ftp_path'] . '/' . THUMB_DIR);
				$result = @ftp_chdir($conn_id, $attach_config['ftp_path'] . '/' . THUMB_DIR);
			}
...
$template->assign_vars(array(
	'ATTACH_VERSION' => sprintf($lang['Attachment_version'], ATTACH_VERSION))
);
