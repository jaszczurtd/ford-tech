...
while( $row = $db->sql_fetchrow($result) )
{
	...
	$name = (strlen(str_replace(array(' ', '&nbsp;'), '', strip_tags($row['html']))) > 2) ? substr(strip_tags($row['html']), 0, 120) : substr(htmlspecialchars($row['html']), 0, 120);
...
$template->assign_vars(array(
	...
	'ADVERT' => htmlspecialchars($board_config['advert']),
	'ADVERT_FOOT' => htmlspecialchars($board_config['advert_foot']),
	...
	'SEPARATOR' => htmlspecialchars($board_config['advert_separator']),
	'SEPARATOR_L' => htmlspecialchars($board_config['advert_separator_l']),
