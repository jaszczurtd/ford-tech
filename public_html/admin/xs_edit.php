...
$editable = array('.htm', '.html', '.tpl', '.css', '.txt', '.cfg', '.xml', '.htaccess');

// get current directory
$current_dir = isset($HTTP_GET_VARS['dir']) ? $HTTP_GET_VARS['dir'] : (isset($HTTP_POST_VARS['dir']) ? $HTTP_POST_VARS['dir'] : 'templates');
$current_dir = str_replace(array("\\", ".."), array("/", ""), $current_dir);
if(substr($current_dir, 0, 9) !== 'templates')
...
include_once('admin_config.' . $phpEx);

if ( !$xs_edit && (isset($HTTP_GET_VARS['filter']) || isset($HTTP_POST_VARS['filter']) || isset($HTTP_POST_VARS['dir']) || isset($HTTP_GET_VARS['dir']) ) )
{
	xs_message($lang['Information'], $lang['xs_edit_permission']);
}
...
{
	$HTTP_GET_VARS['edit'] = basename($HTTP_GET_VARS['edit']);
}
if (isset($HTTP_POST_VARS['edit']))
{
	$HTTP_POST_VARS['edit'] = basename($HTTP_POST_VARS['edit']);
}
...
if(isset($HTTP_GET_VARS['edit']))
{
	...
	$template->assign_vars(array(
		...
		'CONTENT'		=> defined('DEMO_MODE') ? 'you cannot edit file in demo mode' : htmlspecialchars(implode('', @file($localfile))),
		)
	);
	if($current_dir_full)
	{
	...
	for($i=0; $i<count($backups); $i++)
	{
		$template->assign_block_vars('backup', array(
			'TIME'		=> create_date($board_config['default_dateformat'], $backups[$i], $board_config['board_timezone'], true),
...
for($i=0; $i<count($list_files_editable); $i++)
{
	...
	$filetime = $t ? create_date($board_config['default_dateformat'], $t, $board_config['board_timezone'], true) : '&nbsp;';
