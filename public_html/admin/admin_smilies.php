...
*     date modification    : ver. 1.12.3 2005/11/11 14:51
*
...
if ( !function_exists('update_smiles_order') )
{
	function update_smiles_order()
	{
		global $db;

		$sql = "SELECT smilies_id
			FROM " . SMILIES_TABLE . " ORDER by smile_order";
		...
		while( $row = $db->sql_fetchrow($result) )
		{
			...
			{
				message_die(GENERAL_ERROR, "Couldn't update smilies table", "", __LINE__, __FILE__, $sql);
			}
		}
		sql_cache('clear', 'smilies');
	}
}

if ( !function_exists('get_max_order') )
{
	function get_max_order()
	{
		global $db;
		$sql = "SELECT MAX(smile_order) as max_order
			FROM " . SMILIES_TABLE;
		if ( !($result = $db->sql_query($sql)) )
		{
			message_die(GENERAL_ERROR, "Couldn't get smile order", "", __LINE__, __FILE__, $sql);
		}
...
if ( !function_exists('move_smile') )
{
	function move_smile($direction, $id)
	{
		global $db, $phpEx;

		$sql = "SELECT smile_order
			FROM " . SMILIES_TABLE . "
			WHERE smilies_id = " . $id . "
			LIMIT 1";
		if ( !($result = $db->sql_query($sql)) )
		...
		if ( ($direction == 'down' && $current_order != $max_order) || ($direction == 'up' && $current_order > 1) )
		{
			...
			$sql = "UPDATE " . SMILIES_TABLE . "
				SET smile_order = smile_order $first_delim 1
					WHERE smile_order = $current_order $second_delim 1";
			if( !($set_result = $db->sql_query($sql)) )
			{
				message_die(GENERAL_ERROR, "Couldn't update smilies order", "", __LINE__, __FILE__, $sql);
			}
			$sql = "UPDATE " . SMILIES_TABLE . "
				SET smile_order = smile_order $second_delim 1
					WHERE smilies_id = " . intval($id);
			if( !($set_result = $db->sql_query($sql)) )
			{
				message_die(GENERAL_ERROR, "Couldn't update smilies order", "", __LINE__, __FILE__, $sql);
			}
		...
		redirect(append_sid("admin/admin_smilies.$phpEx") . '#' . $id);
	}
}
...
else
{
	...
	if ( !($result = $db->sql_query($sql)) )
	{
		message_die(GENERAL_ERROR, "Couldn't get smile count", "", __LINE__, __FILE__, $sql);
	}
	$row = $db->sql_fetchrow($result);
	if ( $row['total'] != 1 )
	{
...
if ( $HTTP_GET_VARS['move'] && $smiley_id )
{
	move_smile($HTTP_GET_VARS['move'], $smiley_id);
}
...
// Select main mode
//
if( isset($HTTP_GET_VARS['mass_pre_add']) || isset($HTTP_POST_VARS['mass_pre_add']) )
{
	$template->set_filenames(array(
		"body" => "admin/smilies_mass_pre_add.tpl")
	);

	$s_hidden_fields = '<input type="hidden" name="mass_add" value="mass_add" />';
	...
	while( $row = $db->sql_fetchrow($result) )
	{
		$smiley_images_database[] = $row['smile_url'];
	}
	for( $i = 0; $i < count($smiley_images); $i++ )
	{
		for( $j = 0; $j < count($smiley_images_database); $j++ )
		{
			if($smiley_images[$i]==$smiley_images_database[$j])
			{
				$exist = true;
			}
		}
		if(!$exist)
		{
			$template->assign_block_vars('mass_pre_add',array(
				'SMILEY_IMG' => $phpbb_root_path . $board_config['smilies_path'] . '/' . $smiley_images[$i],
...
else if( isset($HTTP_GET_VARS['mass_add']) || isset($HTTP_POST_VARS['mass_add']) )
{
	$y = 0;
	for($i = 0; $i < count($HTTP_POST_VARS['smile_code']); $i++)
	{
		$smile_code = trim(str_replace(array('<', '>'), array('&lt;', '&gt;'), $HTTP_POST_VARS['smile_code'][$i]));
		$smile_url = trim(phpbb_ltrim(basename($HTTP_POST_VARS['smile_url'][$i]), "'"));

		if(!empty($smile_code) && !empty($smile_url))
		{
			$max_order = get_max_order();
			$sql = "INSERT INTO " . SMILIES_TABLE . " (code,smile_url,emoticon,smile_order) VALUES ('" . str_replace("\'", "''", $smile_code) . "','" . str_replace("\'", "''", $smile_url) . "','', " . ($max_order + 1) . ")";
			if( !$result = $db->sql_query($sql) )
			{
				message_die(GENERAL_ERROR, "Couldn't insert new smilies", "", __LINE__, __FILE__, $sql);
			}
			sql_cache('clear', 'smilies');
			$y++;
		}
	}
	$message = sprintf($lang['Click_to_back_smilies'], $y, '<a href="' . append_sid("admin_smilies.$phpEx") . '">', '</a>');
	message_die(GENERAL_INFORMATION, $message);
}
		...
		for( $i = 0; $i < count($fcontents); $i++ )
		{
			...
			for( $j = 2; $j < count($smile_data); $j++)
			{
				...
				if( $smiles[$k] == 1 )
				{
					...
					{
						$sql = "UPDATE " . SMILIES_TABLE . "
							SET smile_url = '" . str_replace("\'", "''", $smile_data[0]) . "', emoticon = ''
				...
				{
					$max_order++;
					$sql = "INSERT INTO " . SMILIES_TABLE . " (code, smile_url, emoticon, smile_order)
						VALUES('" . str_replace("\'", "''", $smile_data[$j]) . "', '" . str_replace("\'", "''", $smile_data[0]) . "', '" . str_replace("\'", "''", $smile_data[1]) . "', $max_order)";
				}
...
else if ( $mode != "" )
{
	switch( $mode )
	{
		...
		case 'edit':
			...
			$smile_data['code'] = str_replace('&lt;', '<', $smile_data['code']);
			$smile_data['code'] = str_replace('&gt;', '>', $smile_data['code']);
		...
		case "save":
			...
			//
			$sql = "UPDATE " . SMILIES_TABLE . "
				SET code = '" . str_replace("\'", "''", $smile_code) . "', smile_url = '" . str_replace("\'", "''", $smile_url) . "', emoticon = ''
		...
		case "savenew":
			...
			$sql = "INSERT INTO " . SMILIES_TABLE . " (code, smile_url, emoticon, smile_order)
				VALUES ('" . str_replace("\'", "''", $smile_code) . "', '" . str_replace("\'", "''", $smile_url) . "', '', " . ($max_order + 1) . ")";
			$result = $db->sql_query($sql);
			if( !$result )
			{
...
else
{
	...
	$template->assign_vars(array(
		...
		"L_MASS_SMILIES_ADD" => $lang['mass_smilies_add'],
		"DELL_ALL_SMILIES" => $lang['dell_all_smilies'],
		"L_UP" => $lang['Move_up'],
	...
	for($i = 0; $i < count($smilies); $i++)
	{
		...
		$template->assign_block_vars("smiles", array(
			...
			"CODE" => htmlspecialchars($smilies[$i]['code']),
			"ID" => $smilies[$i]['smilies_id'],

			"U_SMILEY_UP" => append_sid("admin_smilies.$phpEx?move=up&amp;id=" . $smilies[$i]['smilies_id']),
			"U_SMILEY_DOWN" => append_sid("admin_smilies.$phpEx?move=down&amp;id=" . $smilies[$i]['smilies_id']),
