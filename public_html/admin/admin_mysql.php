...
if ( $HTTP_POST_VARS['this_query'] )
{
	...
	for ($i = 0; $i < count($queries); $i++)
	{
		...
		{
			$message = sprintf($lang['cannot_execute'], htmlspecialchars($cr_query), '<a href="' . append_sid("admin_mysql.$phpEx") . '">', '</a>');
		...
		$j = 0;
			...
			while( $row = $db->sql_fetchrow($result) )
			{
				...
				{
					$body .= '<td class="' . $class . '" align="center">' . htmlspecialchars($row[$keys[$k]]) . '</td>';
				}
				$body .= '</tr>';
				$j++;
...
$template->assign_vars(array(
	'THIS_QUERY' => stripslashes(trim(htmlspecialchars($HTTP_POST_VARS['this_query']))),
