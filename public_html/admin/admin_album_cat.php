...
if( !isset($HTTP_POST_VARS['mode']) )
{
	if( !isset($HTTP_GET_VARS['action']) )
	{
		...
		$template->assign_vars(array(
			...
			'S_MODE' => 'new',
			'L_CREATE_ALBUM' => $lang['Create_album'],
		...
		{
			if ($catrow[$i]['cat_type'] != 0)
			{
				$colspan = 1;
				$add_sc = '<td class="' . (($i % 2) ? 'row1' : 'row2') . '" align="center"><form action="' . append_sid("admin_album_cat.$phpEx") . '" method="post"><input type="hidden" value="new" name="mode" /><input type="hidden" value="' . $catrow[$i]['cat_id'] . '" name="cat_parent"><input name="submit" type="submit" value="' . $lang['Create_sub_album'] . '" class="liteoption"></form></td>';
			}
			else
			{
				$colspan = 2;
			...
			if ($catrow[$i]['cat_type'] != 0)
			{
				...
				$sc_sql = "SELECT * FROM " . ALBUM_CAT_TABLE . "
					WHERE cat_parent = " . $catrow[$i]['cat_id'] . "
					GROUP BY cat_id
					ORDER BY cat_order ASC";
 
				if ( !($sc_result = $db->sql_query($sc_sql)) )
				{
					message_die(GENERAL_ERROR, 'Could not query sub categories', '', __LINE__, __FILE__, $sc_sql);
				}
				...
				while ($subcatrow = $db->sql_fetchrow($sc_result))
				{
					$template->assign_block_vars('catrow', array(
						...
						'ADD_SC' => '',
						'COLOR' => ($i % 2) ? 'row1' : 'row2',
						'TITLE' => "&nbsp;&nbsp;&nbsp;&nbsp;" . $subcatrow['cat_title'],
						'DESC' => "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" . preg_replace("#\n#", "<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;", $subcatrow['cat_desc']),
						'S_MOVE_UP' => append_sid("admin_album_cat.$phpEx?action=move&amp;move=-15&amp;cat_parent=" . $catrow[$i]['cat_id'] . "&amp;cat_id=" . $subcatrow['cat_id']),
						'S_MOVE_DOWN' => append_sid("admin_album_cat.$phpEx?action=move&amp;move=15&amp;cat_parent=" . $catrow[$i]['cat_id'] . "&amp;cat_id=" . $subcatrow['cat_id']),
						'S_EDIT_ACTION' => append_sid("admin_album_cat.$phpEx?action=edit&amp;cat_id=" . $subcatrow['cat_id']),
						'S_DELETE_ACTION' => append_sid("admin_album_cat.$phpEx?action=delete&amp;cat_id=" . $subcatrow['cat_id'])
						)
					);
				}
			}
	...
	else
	{
		...
		else if( $HTTP_GET_VARS['action'] == 'move' )
		{
			...
			reorder_cat();
			$message = '<meta http-equiv="refresh" content="0;url=' . append_sid("admin_album_cat.$phpEx", true) . '">' . $lang['Config_updated'] . "<br /><br />" . sprintf($lang['Click_return_album_category'], "<a href=\"" . append_sid("admin_album_cat.$phpEx") . "\">", "</a>") . "<br /><br />" . sprintf($lang['Click_return_admin_index'], "<a href=\"" . append_sid("index.$phpEx?pane=right") . "\">", "</a>");
...
{
	if ( ( $HTTP_POST_VARS['mode'] == 'new' ) || ($HTTP_POST_VARS['mode'] == 'newcat') )
	{
		if( !isset($HTTP_POST_VARS['cat_title']) )
		{
			...
			if ( !isset($HTTP_POST_VARS['cat_parent']) )
			{
				$cat_parent = '';
			}
			else
			{
				$cat_parent = "<input type=\"hidden\" value=\"" . $HTTP_POST_VARS['cat_parent'] . "\" name=\"cat_parent\">";
			}
 
			if ($HTTP_POST_VARS['mode'] == 'newcat')
			{
				$cat_type = '<input type="hidden" value="1" name="cat_type" />';
			}
		...
		else
		{
			...
			{
				$cat_parent = intval($HTTP_POST_VARS['cat_parent']);
			}
			if ( !isset($HTTP_POST_VARS['cat_type']) )
			{
				$cat_type = 0;
			}
			else
			...
			$sql = "INSERT INTO ". ALBUM_CAT_TABLE ." (cat_title, cat_desc, cat_order, cat_view_level, cat_upload_level, cat_rate_level, cat_comment_level, cat_edit_level, cat_delete_level, cat_approval, cat_parent, cat_type)
					VALUES ('$cat_title', '$cat_desc', '$cat_order', '$view_level', '$upload_level', '$rate_level', '$comment_level', '$edit_level', '$delete_level', '$cat_approval', $cat_parent, $cat_type)";
			if(!$result = $db->sql_query($sql))
			{
