...
if ( @$_GET['custom'] == 'edit' || @$_POST['custom'] == 'edit' )
{
	define('MODULE_ID', 67);
}
if ( @$_GET['custom'] == 'delete' || @$_POST['custom'] == 'delete' )
{
	define('MODULE_ID', 68);
}
else
...
if( !empty($setmodules) )
{
    ...
    $module['Download2']['Afield'] = "$file?custom=add";
    $module['Download2']['Efield'] = "$file?custom=edit";
    $module['Download2']['Dfield'] = "$file?custom=delete";
	return;
}
	...
	switch($custom)
	{
		case 'add':
		{
			...
			if ($add == 'do') 
			{
				...
				$sql = "INSERT INTO " . PA_CUSTOM_TABLE . " (custom_name, custom_description) VALUES('" . $form['name'] . "', '" . $form['description'] . "')";
