...
$text_types = array('.tpl', '.htm', 'html', '.txt', '.css', '.cfg', '.xml');
...
// other allowed to import extensions
$allowed_types = array('.tps', '.js', '.htaccess', '.swf', '.psd');

$file = XS_TEMP_DIR . $filename;
...
while($pos < strlen($str))
{
	...
	$data['ext'] = strrchr($data['filename'], '.');
	if($write_local)
	{
	...
	else
	{
		if($data['filename'])
		{
			if(!$list_only)
			{
				if (xs_in_array($data['ext'], $text_types) || xs_in_array($data['ext'], $img_types) || xs_in_array($data['ext'], $allowed_types))
				{
					if($write_local)
					{
