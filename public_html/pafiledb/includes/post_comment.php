...
if(!isset($HTTP_POST_VARS['submit']))
{
	...
	$links_status = ( $config['allow_comment_links']  ) ? $lang['Links_are_ON'] : $lang['Links_are_OFF'];
	$images_status = ( $config['allow_comment_images']  ) ? $lang['Images_are_ON'] : $lang['Images_are_OFF'];
	...
	$locbar_comment = '<span class="nav"><a href="' . append_sid("index.$phpEx") . '" class="nav">' . $board_config['sitename'] . '</a> &raquo; <a href="' . append_sid("dload.php") . '" class="nav">Download</a> &raquo; <a href="' . append_sid("dload.php?action=category&amp;id=" . $file['file_catid']) . '" class="nav">' . $category['cat_name'] . '</a> &raquo; <a href="'.append_sid('dload.php?action=file&id='.$file['file_id']).'" class="nav">'.$file['file_name']. '</a> &raquo; ' . $lang['Comment_add'].'</span>';
	...
	$template->assign_vars(array(
		...
		'LOCBAR_COMMENT' => locbar($locbar_comment),
	...
	{
		$comments_text = $HTTP_POST_VARS['message'];
		$title = htmlspecialchars($HTTP_POST_VARS['subject']);
...
//
if(isset($HTTP_POST_VARS['submit']) && $userdata['session_logged_in'])
{
	...
	$sql = "INSERT INTO " . PA_COMMENTS_TABLE . " (file_id, comments_text, comments_title, comments_time, comment_bbcode_uid, poster_id) VALUES('" . $id . "','" . str_replace("\'", "''", $comments_text) . "','" . str_replace("\'", "''", $title) . "','" . CR_TIME . "', '" . $comment_bbcode_uid . "','" . $poster_id . "')";
	if ( !($db->sql_query($sql)) )
	{
