...
if ( defined('PHPBB_INSTALLED') )
{
	...
	if ( $mode == 'sql' && $userdata['user_level'] == ADMIN )
	{
		...
		&nbsp; <a href="' . $cf . '?mode=sql&amp;type=moved">' . $lang['moved_topics'] . '</a><br />
		...
		if ( $type == 'users_posts' || $all )
		{
			...
			{
				$list_users_sql = implode(', ', $list_users);
		...
		if ( ($type == 'users' || $all) )
		{
			...
			if ( $g_users )
			{
				...
				$print_result .= '<tr><td><br /><br />' . sprintf($lang['users_without_groups'], count($g_users), $gecho_ids, '<a href="' . $cf . '?mode=sql&amp;type=users&amp;repair_ug=1">', '</a>') . '</td></tr>';
			}
		}
	...
	echo '<tr><td>MySql ping</td><td>: ' . (($sql_ping = @mysql_ping($db->db_connect_id)) ? $sql_ping . ' ms' : '-') . ', ' . $cf_cache_work . '</b></td></tr>';
