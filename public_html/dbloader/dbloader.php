...
if( $config_size_ok ) // config w porz±dku, po³±cz z baz± danych
{
	...
	elseif( !mysql_query("USE `" . escape_str($dbname) . "`") )
	{
		$db_error = true;
		$db_db_err = true;
	}
	else
	{
		...
		$db_error = (!$db->db_connect_id) ? true : false;
	}
}
else
{
...
elseif( $db_error )
{
	...
	if ( !$db_conn_err && !$db_db_err )
	{
		$sql_error = $db->sql_error();
	}
	else
	{
		$sql_error['code'] = mysql_errno();
		$sql_error['message'] = mysql_error();
	}
...
VALUES ( '".$errors[$i]['pos']."', '".$errors[$i]['code']."', '".escape_str($errors[$i]['msg'])."')";
		...
		foreach($queries as $query)
		{
			...
			{
				$err = $db->sql_error();
				echo 'B³±d zapisu konfiguracji ('.$err['code'].': '.htmlspecialchars($err['message']).')<br />'.htmlspecialchars($query).'<hr />';
				$save_error = true;
			}
		}
	...
	elseif( $step == 'show_errors' )
	{
		...
		for($i = 0; $i < $rt_data['errors']; $i++)
		{
			...
			$template->assign_block_vars('dbread.error', array(
				...
				'QUERY' => nl2br(htmlspecialchars($query['query'])))
			);
		}
		$template->assign_vars(array(
