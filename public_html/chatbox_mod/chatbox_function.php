...
function write_msg($nick, $msg)
{
	...
	{
		write_msg(_CHATBOX_SYSTEM_MSG,"<span style='color: #006600'><b><i>$nick</b> joined chat on " . create_date($board_config['default_dateformat'], CR_TIME, $board_config['board_timezone']) . "</i></span>");
	...
	$sql = "INSERT INTO ".$table_chatbox_name." VALUES ('','".mysql_real_escape_string($nick)."','".mysql_real_escape_string($msg)."','".CR_TIME."')";
...
function get_msg($start,$number,$dir)
{
	...
	$sql = "SELECT id,name,msg,timestamp FROM " . $table_chatbox_name . " ORDER BY id LIMIT ". intval($start) . ", " . intval($number);
...
function user_join($usernick,$status)
{
	...
	$sql = "DELETE FROM ".$table_chatbox_session_name." WHERE username = '".mysql_real_escape_string($usernick)."'";
    ...
    $sql = "INSERT INTO ".$table_chatbox_session_name." (username, lastactive, laststatus) VALUES ('" . mysql_real_escape_string($usernick) . "', '".CR_TIME."', '" . mysql_real_escape_string($status) . "')";
...
function user_active($usernick)
{
    ...
    $sql = "UPDATE ".$table_chatbox_session_name." SET lastactive = '".CR_TIME."' WHERE username = '".mysql_real_escape_string($usernick)."'";
...
function isuser_active($usernick)
{
    ...
    $sql = "SELECT lastactive FROM " . $table_chatbox_session_name . " WHERE username = '" . mysql_real_escape_string($usernick) . "'";
	...
	{
		if ( ( ($lastactive + $chatbox_config['away_time']) < CR_TIME ) and ( ($lastactive + $chatbox_config['offline_time']) > CR_TIME ) )
		{
			$result = "away";
		}
		else if ( ($lastactive + $chatbox_config['offline_time']) <= CR_TIME )
		{
			drop_user($usernick);
			$result = "offline";
		}
...
function drop_user($usernick)
{
	...
	if ($lastactive != "")
	{
		...
		write_msg(_CHATBOX_SYSTEM_MSG, "<span style='color: #660000'><i><b>" . $usernick . "</b> logged out on " . create_date($board_config['default_dateformat'], CR_TIME, $board_config['board_timezone'])."</i></span>");
	}
}
