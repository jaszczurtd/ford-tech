...
function show_msg()
{
	...
	for ( $count = 0; $count < count($records); $count++ )
	{
		...
		if ( (($msgs['time']) + $chatbox_config['delete_time']) < CR_TIME )
		{
...
function chatters_sub() // This function checks status of user
{
	...
	for($s = 1; $s <= $isimler[0]; $s++)
	{
		...
		if($currentact != $activity[$s])
		{
			...
			{
				write_msg(_CHATBOX_SYSTEM_MSG, "<span style='color: #006666'><i><b>$isimler[$s]</b> became away on " . create_date($board_config['default_dateformat'], CR_TIME, $board_config['board_timezone']) . "</i>");
			}
			
			$sql = "UPDATE " . $table_chatbox_session_name . " SET laststatus = '" . mysql_real_escape_string($currentact) . "' WHERE username = \"" . mysql_real_escape_string($isimler[$s]) . "\"";
...
function clear_chat()
{
    ...
    if (!$result = $db->sql_query($sql))
	{
		die("SQL Error in function clear_chat()");
	}
}
