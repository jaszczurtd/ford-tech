 ...
 *   date modification    : ver. 1.12.4 2005/10/24 23:54
 *
...
while ( $row = $db->sql_fetchrow($result) )
{
	...
	if ( $view_online )
	{
		if ( $row['session_page'] < 1 || !$tree['auth'][POST_FORUM_URL . $row['session_page']]['auth_view'] )
		{
			switch( $row['session_page'] )
			{
				...
				case PAGE_TOPIC_VIEW:
					...
					break;
				case PAGE_PRIVMSGS:
					$location = ($user_id != ANONYMOUS) ? $lang['Viewing_priv_msgs'] : $lang['Forum_index'];
				...
				case PAGE_SHOUTBOX:
					$location = 'ShoutBox';
					$location_url = "shoutbox_view.$phpEx";
					break;
		...
		$template->assign_block_vars("$which_row", array(
			...
			'ROW_CLASS' => $row_class,
			'USERNAME' => $username,
			'USERNAME_STYLE' => $colored_username[1],
			'LASTUPDATE' => sprintf($lang_online, $time_online),
