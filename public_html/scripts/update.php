 ...
 *   version        : ver. 1.12.7 2012/07/13 20:38
 *
 ***************************************************************************/

define('FILE_CHECKSUM', 'd2f1932b5017f17a391f0adc421b4953');
...
else
{
	...
	for($i = 0; $i < count($sql_query); $i++)
	{
		if ( $file )
		{
			$file_query .= htmlspecialchars($sql_query[$i]) . ";<br /><br />";
		}
		else
		{
			if ( !($result = $db->sql_query($sql_query[$i])) )
			{
				...
				$message .= '<tr><td class="row' . $row . '"><span class="gensmall">' . htmlspecialchars($cur_query) . '<br /><u><b>' . $lang['failed'] . '</b></u> - <b>' . htmlspecialchars($error['message']) . '</b></span></td></tr>';
				$no_ok++;
			} else $ok++;
		}
	}
	...
	foreach($tables_keys as $table => $keys)
	{
		...
		for($i = 0; $i < count($keys); $i++)
		{
			if ( !is_array($existing_keys) || !@in_array($keys[$i], $existing_keys) )
			{
				...
				if ( $file )
				{
					$file_query .= htmlspecialchars($sql) . ";<br /><br />";
				}
				else
				{
					if ( !($result = $db->sql_query($sql)) )
