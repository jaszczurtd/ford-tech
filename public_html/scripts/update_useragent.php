...
$sql_where = "WHERE user_agent IS NOT NULL
	AND user_agent <> ''
	AND user_agent NOT LIKE '%.gif\";%'
	AND user_agent NOT LIKE '%.jpg\";%'
	AND user_agent NOT LIKE '%.jpeg\";%'
	AND user_agent NOT LIKE '%.png\";%'";
...
while ($i <= $max_posts)
{
	...
	while ( $row = $db->sql_fetchrow($result) )
	{
		...
		$sqlu = "UPDATE " . POSTS_TABLE . "
			SET user_agent='" . str_replace("'", "''", serialize(user_agent($user_agent))) . "'
