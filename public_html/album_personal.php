...
if ($total_pics > 0)
{
	...
	$sql = "SELECT p.pic_id, p.pic_filename, p.pic_title, p.pic_desc, p.pic_user_id, p.pic_user_ip, p.pic_time, p.pic_view_count, p.pic_lock, r.rate_pic_id, AVG(r.rate_point) AS rating, COUNT(DISTINCT c.comment_id) AS comments, MAX(c.comment_id) as new_comment
	...
	for ($i = 0; $i < count($picrow); $i += $album_config['cols_per_page'])
	{
		...
		for ($j = $i; $j < ($i + $album_config['cols_per_page']); $j++)
		{
			...
			$pic_size = @getimagesize(ALBUM_UPLOAD_PATH . $picrow[$j]['pic_filename']);
			...
			else $u_pic = append_sid("album_page.$phpEx?pic_id=". $picrow[$j]['pic_id']);
			...
			$template->assign_block_vars('picrow.pic_detail', array(
				...
				'LINK'  => (append_sid("http://www.ford-tech.pl/album_pic.$phpEx?pic_id=". $picrow[$j]['pic_id'])),
				...
				'IP' => ($userdata['user_level'] == ADMIN) ? $lang['IP_Address'] . ': <a href="' . $board_config['address_whois'] . '' . decode_ip($picrow[$j]['pic_user_ip']) . '" target="_blank">' . decode_ip($picrow[$j]['pic_user_ip']) .'</a><br />' : ''
				)
			);
		}
	}
	...
	// Pagination
	// --------------------------------
	generate_pagination(append_sid("album_personal.$phpEx?user_id=$user_id&amp;sort_method=$sort_method&amp;sort_order=$sort_order"), $total_pics, $pics_per_page, $start);
}
else
{
