 ...
 *   date modification    : ver. 1.12.0 2005/10/10 2:21
 *
...
// -------------------------
//
function generate_user_info(&$row, $date_format, $group_mod, &$from, &$posts, &$joined, &$poster_avatar, &$profile_img, &$profile, &$search_img, &$search, &$pm_img, &$pm, &$email_img, &$email, &$www_img, &$www, &$icq_status_img, &$icq_img, &$icq, &$msn_img, &$msn, &$yim_img, &$yim)
{
...
if ( $group_id && isset($HTTP_POST_VARS['mass_email']) )
{
	redirect(append_sid("groupcp_mail.$phpEx?" . POST_GROUPS_URL . "=" . $group_id, true));
}
...
else if ( isset($HTTP_POST_VARS['joingroup']) && $group_id )
{
	...
	{
		$is_autogroup_enable = ($row['group_count'] <= $userdata['user_posts']) ? true : false;
	...
	$emailer->set_subject(sprintf($lang['Group_request'], $moderator['group_name']));
...
else if ( isset($HTTP_POST_VARS['unsub']) || isset($HTTP_POST_VARS['unsubpending']) && $group_id )
{
	...
	$row = $db->sql_fetchrow($result);
	if ( $row['group_no_unsub'] )
	{
		message_die(GENERAL_MESSAGE, 'You can not leave this group');
	}
...
else if ( $group_id )
{
	...
	//
	$sql = "SELECT g.group_moderator, g.group_type, g.group_mail_enable, aa.auth_mod
		FROM " . GROUPS_TABLE . " g
		...
		if ( !empty($HTTP_POST_VARS['add']) || !empty($HTTP_POST_VARS['remove']) || isset($HTTP_POST_VARS['approve']) || isset($HTTP_POST_VARS['deny']) )
		{
			...
			if ( isset($HTTP_POST_VARS['add']) )
			{
				...
				if ( !($row = $db->sql_fetchrow($result)) )
				{
					...
					$message = $lang['No_user_id_specified'] . "<br /><br />" . sprintf($lang['Click_return_group'], "<a href=\"" . append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id") . "\">", "</a>") . "<br /><br />" . sprintf($lang['Click_return_index'], "<a href=\"" . append_sid("index.$phpEx") . "\">", "</a>");
				...
				if ( !($db->sql_fetchrow($result)) )
				{
					...
					$emailer->set_subject(sprintf($lang['Group_added'], $group_name));
	...
	//
	$sql = "SELECT username, user_id, user_viewemail, user_posts, user_regdate, user_from, user_website, user_email, user_icq, user_yim, user_msnm  
		FROM " . USERS_TABLE . " 
	...
	if ( !$group_moderator['user_id'] )
	{
		message_die(GENERAL_ERROR, 'Group moderator not exist !');
	}
	...
	if ( $group_info['group_count'] && $group_info['group_count_enable'] )
	{
		$sql = "SELECT ug.user_id
			...
			WHERE ug.group_id = $group_id
				...
				AND u.user_id <> " . $group_info['group_moderator'] . "
				AND u.user_posts < " . $group_info['group_count'];
		if ( !($result = $db->sql_query($sql)) )
		{
			message_die(GENERAL_ERROR, 'Error geting users groups data', '', __LINE__, __FILE__, $sql);
		}
		$user_ids = '';
		while ($rem_row = $db->sql_fetchrow($result))
		{
			$user_ids .= ($user_ids) ? ', ' . $rem_row['user_id'] : $rem_row['user_id'];
		}
	...
	$sql = "SELECT u.username, u.user_id, u.user_viewemail, u.user_posts, u.user_regdate, u.user_from, u.user_website, u.user_email, u.user_icq, u.user_yim, u.user_msnm, ug.user_pending 
	...
	$sql = "SELECT u.username, u.user_id, u.user_viewemail, u.user_posts, u.user_regdate, u.user_from, u.user_website, u.user_email, u.user_icq, u.user_yim, u.user_msnm
	...
	$is_autogroup_enable = ($group_info['group_count'] <= $userdata['user_posts']) ? true : false;
	...
	$template->assign_vars(array(
		...
		'L_MASS_EMAIL' => $lang['GP_mail_title'],
		...
		'GROUP_COLOR' => (($group_info['group_type'] == GROUP_HIDDEN && !$is_group_member && !$is_moderator) || !$group_info['group_color']) ? '' : ' style="color: #' . $group_info['group_color'] . '"',
		'GROUP_STYLE' => (($group_info['group_type'] == GROUP_HIDDEN && !$is_group_member && !$is_moderator) || !$group_info['group_style']) ? '' : ' style="' . $group_info['group_style'] . '"',
		'GROUP_DESC' => ($group_info['group_type'] == GROUP_HIDDEN && !$is_group_member && !$is_moderator) ? '' : $group_info['group_description'],
	...
	//
	for($i = $start; $i < min($user_topics_per_page + $start, $members_count); $i++)
	{
		$username = $group_members[$i]['username'];
		$user_id = $group_members[$i]['user_id'];
		$member_row = $group_members[$i];
	...
	$current_page = ( !$members_count ) ? 1 : ceil( $members_count / $user_topics_per_page );

	if ( $members_count > $user_topics_per_page )
	{
		generate_pagination("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id", $members_count, $user_topics_per_page, $start);
	}
	...
	if ( $is_moderator )
	{
		...
		if ( $modgroup_pending_count )
		{
			for($i = 0; $i < $modgroup_pending_count; $i++)
			{
				...
				$modgroup_row = $modgroup_pending_list[$i];
	...
	if ( $is_moderator )
	{
		...
		if ( ($group_info['group_moderator'] == $userdata['user_id'] && $group_info['group_mail_enable'] == 1) || $userdata['user_level'] == ADMIN )
		{
			$template->assign_block_vars('switch_mod_option.switch_mass_email', array());
		}
...
else
{
	...
	if ( $userdata['session_logged_in'] ) 
	{
		$sql = "SELECT g.*, ug.user_pending 
			FROM (" . GROUPS_TABLE . " g, " . USER_GROUP_TABLE . " ug)
			WHERE ug.user_id = " . $userdata['user_id'] . "  
				AND ug.group_id = g.group_id
				AND g.group_single_user <> " . TRUE . "
			ORDER BY g.group_order, ug.user_id";
		if ( !($result = $db->sql_query($sql)) )
		{
			message_die(GENERAL_ERROR, 'Error getting group information', '', __LINE__, __FILE__, $sql);
		}
		...
		{
			$in_group = array();
			$s_member_groups_opt = array();
			do
			{
				$in_group[] = $row['group_id'];
				$s_member_groups_opt[] = $row;
			}
			while( $row = $db->sql_fetchrow($result) );
		}
	}
	...
	if ( count($s_group_list_opt) || count($s_member_groups_opt) )
	{
		...
		if ( count($s_member_groups_opt) )
		{
			$template->assign_block_vars('switch_groups_joined', array() );
		}

		if ( count($s_group_list_opt) )
		{
			$template->assign_block_vars('switch_groups_remaining', array() );
		}

		for($i = 0; $i < count($s_member_groups_opt); $i++)
		{
			$row = $s_member_groups_opt[$i];

			$template->assign_block_vars('switch_groups_joined.group_member', array(
				'U_GROUP' => append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=".$row['group_id']),
				'GROUP_COLOR' => ($row['group_color']) ? ' style="color: #' . $row['group_color'] . '"' : '',
				'GROUP_STYLE' => ($row['group_style']) ? ' style="' . $row['group_style'] . '"' : '',
				'L_PENDING' => ($row['user_pending']) ? '<span class="gensmall""><br />(' . $lang['Memberships_pending'] . ')</span>' : '',
				'GROUP_DESC' => $row['group_description'],
				'GROUP_NAME' => $row['group_prefix'] . $row['group_name'])
			);
		}

		for($i = 0; $i < count($s_group_list_opt); $i++)
		{
			$row = $s_group_list_opt[$i];

			$template->assign_block_vars('switch_groups_remaining.group_list', array(
				'U_GROUP' => append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=".$row['group_id']),
				'GROUP_COLOR' => ($row['group_color']) ? ' style="color: #' . $row['group_color'] . '"' : '',
				'GROUP_STYLE' => ($row['group_style']) ? ' style="' . $row['group_style'] . '"' : '',
				'GROUP_DESC' => $row['group_description'],
		...
		$template->assign_vars(array(
			...
			'L_JOIN_A_GROUP' => $lang['Usergroups'],
			'L_UNSUBSCRIBE' => $lang['Unsubscribe'])
		);
