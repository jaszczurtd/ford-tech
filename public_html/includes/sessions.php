...
function make_over_userdata($userdata)
{
	...
	unset($userdata['user_password'], $userdata['user_regdate'], $userdata['user_actkey'], $userdata['user_newpasswd'], $userdata['user_block_by'], $userdata['user_ip'], $userdata['user_spend_time'], $userdata['user_visit']);

	$_cookie_config = md5('_cookie_config' . $userdata['user_id']);

	$user_config = unserialize(stripslashes($HTTP_COOKIE_VARS[$unique_cookie_name . $_cookie_config]));
	...
	$userdata['user_topics_per_page'] = (intval($userdata['user_topics_per_page']) < 1) ? $board_config['topics_per_page'] : intval($userdata['user_topics_per_page']);
	$userdata['user_posts_per_page'] = (intval($userdata['user_posts_per_page']) < 1) ? $board_config['posts_per_page'] : intval($userdata['user_posts_per_page']);
