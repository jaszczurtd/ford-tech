 ...
 *   date modification    : ver. 1.12.0 2005/10/10 2:10
 *                                                          
...
function auth($type, $forum_id, $userdata, $f_access = '')
{
	...
	if( !isset($auth_buffer) )
	{
		$auth_buffer = array();
	}
	if( empty($f_access) )
	{
		$auth_path = $type . '_' . $forum_id . '_' . $userdata['user_id'];
		if( isset($auth_buffer[$auth_path]) )
		{
			return $auth_buffer[$auth_path];
		}
	}
	else
	{
	...
	switch( $type )
	{
		case AUTH_ALL:
			...
			$auth_fields = array('auth_view', 'auth_read', 'auth_post', 'auth_reply', 'auth_edit', 'auth_delete', 'auth_sticky', 'auth_announce', 'auth_globalannounce', 'auth_vote', 'auth_pollcreate');
			break;
	...
	if ( empty($f_access) )
	{
		...
		if ( !($f_access = $db->$sql_fetchrow($result)) && !defined('IN_ADMIN') && !defined('IN_LOGIN') )
		{
	...
	for($i = 0; $i < count($auth_fields); $i++)
	{
		...
		{
			for($k = 0; !empty($f_access) && $k < count($f_access); $k++)
			{
	...
	if ( $auth_path )
	{
		$auth_buffer[$auth_path] = $auth_user;
	}
...
function auth_check_user($type, $key, $u_access, $is_admin)
{
	...
	if ( count((array)$u_access) )
	{
		for($j = 0; $j < count((array)$u_access); $j++)
		{
			$result = 0;
			switch($type)
			{
