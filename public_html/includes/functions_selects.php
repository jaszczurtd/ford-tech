...
function date_format_select($default, $timezone, $select_name = 'dateformat')
{
	...
	$timezone == ( !isset($timezone) ) ? $board_config['board_timezone'] : $timezone;	$now = CR_TIME + (3600 * $timezone);
	...
	for ($i = 0; $i < sizeof($date_formats); $i++)
	{
		...
		$df_select .= '>' . strtr($display, $translate) . '</option>';
	}
	$df_select .= '</select>';
