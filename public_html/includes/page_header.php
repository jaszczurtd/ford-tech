...
if( $forum_id )
    {
        $logo_sql = "SELECT forum_logo FROM " . FORUMS_TABLE . " WHERE forum_id = " . $forum_id;
            if ( !($result = $db->sql_query($logo_sql)) )
                { message_die(GENERAL_ERROR, 'Could not obtain forum logo', '', __LINE__, __FILE__, $logo_sql); }
        $logo = $db->sql_fetchrow($result);
    }

$logo = ( $logo['forum_logo'] ) ? '<img src="templates/' . str_replace('.css', '', $theme['head_stylesheet']) . '/images/forum_logos/' . $logo['forum_logo'] . '"  border="0" alt="{L_INDEX_PORTAL}" vspace="1">' : '<img src="templates/' . str_replace('.css', '', $theme['head_stylesheet']) . '/images/logo3.png" border="0" alt="{L_INDEX_PORTAL}" vspace="1">';
...
$template->assign_vars(array(
	...
	'U_PERSONAL_ALBUM' => append_sid("album_personal.$phpEx?user_id=". $userdata['user_id']),
...
if ( !$advert_hide )
{
	...
	if ( $check_cache = sql_cache('check', 'advertising') )
	{
		include($check_cache);
	}
...
if ( $board_config['clog'] )
{
	...
	if ( isset($_SERVER['REQUEST_URI']) )
	{
		...
		$addr_info = htmlspecialchars($addr_info);
	}
	else
	{
		if ( !$forum_id == '' )
	...
	if ( $board_config['clog'] == 2 )
	{
		if ( count($_POST) > 0 )
		{
			...
			while( list($key, $val) = @each($_POST) )
			{
				if ( $val != '' && !in_array($key, $no_log_key) && !preg_match('/addbbcode/i', $key)  )
				{
					...
					$post_log_data .= $key . '=' . htmlspecialchars(stripslashes($val)) . ' | ';
				}
			}
		...
		if ( count($_COOKIE) > 0 )
		{
			...
			{
				$cookie_log_data .= htmlspecialchars(stripslashes($val)) . ' | ';
			}
