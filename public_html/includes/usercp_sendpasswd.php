 ...
 *   date modification    : ver. 1.12.0 2005/09/19 19:03
 *
...
if ( isset($HTTP_POST_VARS['submit']) )
{
	...
	$sql = "SELECT user_id, username, user_email, user_emailtime, user_active, user_lang 
		FROM " . USERS_TABLE . " 
	...
	if ( $result = $db->sql_query($sql) )
	{
		if ( $row = $db->sql_fetchrow($result) )
		{
			...
			if ( CR_TIME - $row['user_emailtime'] < 900 )
			{
			...
			$sql = "UPDATE " . USERS_TABLE . " 
				SET user_newpasswd = '" . phpbb_hash($user_password) . "', user_actkey = '$user_actkey', user_emailtime = " . CR_TIME . "
