...
{
	$SID1 = ( isset($HTTP_GET_VARS['sid']) ) ? $HTTP_GET_VARS['sid'] : '';
	$verify_user_value = $userdata['session_ip'];
	$verify_user_value .= $userdata['user_lastvisit'];
	$verify_user_value .= "salt69";

	$verify_user = hash("sha1", $SID1.$verify_user_value);

	if (!preg_match('/^[A-Za-z0-9]*$/', $SID1, $res) || $SID1 != $userdata['session_id']) 
	{
		message_die(GENERAL_MESSAGE, $lang['Invalid_session']);
	}
}
   ...
   )
{
	...
	if ( $mode == 'editprofile' )
	{
		...
		$SID2 = ( isset($HTTP_POST_VARS['session_id']) && !is_array($HTTP_POST_VARS['session_id']) ) ? $HTTP_POST_VARS['session_id'] : '';
		$veryfy_user_value = ( !empty($HTTP_POST_VARS['verify_user']) ) ?  $HTTP_POST_VARS['verify_user'] : '';
		if (!preg_match('/^[A-Za-z0-9]*$/', $SID2, $res) || $SID2 != $userdata['session_id'] || $verify_user != $veryfy_user_value) 
		{
			message_die(GENERAL_MESSAGE, $lang['Invalid_session']);
		}
	}
	...
	else
	{
		...
		$custom_color = ( isset($HTTP_POST_VARS['custom_color']) ) ? htmlspecialchars($HTTP_POST_VARS['custom_color']) : $userdata['user_custom_color'];
	}
	...
	if ( $mode == 'editprofile' || $mode == 'register' )
	{
		...
		if ( $custom_fields_exists )
		{
			...
			for($i = 0; $i < count($custom_fields[0]); $i++)
			{
				...
				if ( $custom_fields[5][$i] && empty($split_field_test) )
				{
					$error = TRUE;
	...
	if ( !empty($new_password) && !empty($password_confirm) )
	{
		...
		else
		{
			if ( $mode == 'editprofile' )
			{
				...
				if ( !phpbb_check_hash($cur_password, $row['user_password']) )
				{
					$error = TRUE;
