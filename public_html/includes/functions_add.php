...
function birthday_list()
{
	...
	if ( $birthday_data[0] < ($current_time - 3600) )
	{
		...
		$sql = "SELECT user_id, username, user_email, user_lang, user_birthday, user_level, user_jr
			FROM " . USERS_TABLE. "
	...
	for($i = 0; $i < count($birthday_data); $i++)
	{
		...
		if ( $user_birthday2 == $date_today )
		{
			...
			$congratulations_link = (in_array($birthday_data[$i]['user_id'], $check_user_ids) || $birthday_data[$i]['user_id'] == $userdata['user_id']) ? '' : ' <a href="' . append_sid("index.$phpEx?mode=congratulations&amp;user=" . $birthday_data[$i]['user_id'] . "&amp;user_age=" . $user_age . "") . '"' . $colored_username[1] . ' class="gensmall">(' . $lang['send_congratulations'] . ')</a>';
	...
	return array($birthday_today_list, $birthday_week_list);
}
