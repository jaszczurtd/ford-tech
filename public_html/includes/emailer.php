...
/***************************************************************************
    ...
    date modification    : ver. 1.12.5 2005/10/06 12:40
...
class emailer
{
	var $msg, $subject, $extra_headers, $use_html;
	...
	function reset()
	{
		...
		$this->vars = $this->msg = $this->extra_headers = $this->use_html = '';
	}
	...
	function send()
	{
		...
		if ( !$board_config['board_email_form'] && !$board_config['smtp_delivery'] )
		{
			return;
		}
		...
		$email_from = ($this->from != '') ? "From: $this->from\n" : "From: $admin_email\n";
		$email_from = ($this->from != '') ? "From: $this->from <$this->from>\n" : "From: $admin_email <$admin_email>\n";
		$email_reply_to = ($this->reply_to != '') ? "Reply-to: $this->reply_to\n" : "Reply-to: $admin_email\n";
		$email_return_path = ($this->reply_to != '') ? "Return-Path: $this->reply_to\n" : "Return-Path: $admin_email\n";

		$this->extra_headers = $email_reply_to . $email_from . $email_return_path . "Message-ID: <" . md5(uniqid(time())) . "@" . $board_config['server_name'] . ">\nMIME-Version: 1.0\nContent-type: text/" . (($this->use_html) ? "html" : "plain") . "; charset=" . $this->encoding . "\nContent-transfer-encoding: 8bit\nDate: " . date('r', time()) . "\nX-Priority: 3\nX-MSMail-Priority: Normal\nX-Mailer: PHP\nX-MimeOLE: Produced By phpBB2\n" . $this->extra_headers . (($cc != '') ? "Cc: $cc\n" : '')  . (($bcc != '') ? "Bcc: $bcc\n" : ''); 
		...
		if ( !$test )
		{
			...
			else
			{
				...
				if (!$result && $board_config['sendmail_fix'] && $empty_to_header)
				{
					...
					update_config('sendmail_fix', '0');

					$board_config['sendmail_fix'] = 0;
					$result = @mail($to, $this->subject, preg_replace("#(?<!\r)\n#s", "\n", $this->msg), $this->extra_headers);
				}
			}
		}
		...
		{
			print '<b>Title</b>: ' . $this->subject . '<br /><b>Text</b>: ' . $this->msg . '<br /><b>To</b>: ' . $to . '<br /><b>Headers</b>: ' . str_replace("\n", "<br />", $this->extra_headers);
		}
		// Did it work?
		if (!$result && !$test)
		{
			print '<center><b>' . $lang['failed_sending_email'] . '</b></center><br />';
			return false;
		}
		return true;
	}
