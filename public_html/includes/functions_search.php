...
{
	static $drop_char_match = array('^', '$', '&', '(', ')', '<', '>', '`', '\'', '"', '|', ',', '?', '%', '~', '+', 'www.', 'http://', '[', ']', '{', '}', ':', '\\', '/', '=', '#', '\'', ';', '!');
...
function add_search_words($mode, $post_id, $post_text, $post_title = '', $post_title_e = '')
{
	...
	$search_raw_words['title_e'] = split_words(clean_words('post', $post_title_e, $stopword_array, $synonym_array));
...
function username_search($search_match)
{
	...
	{
		$search_match = str_replace(array('%', '_'), array('\%', '\_'), $search_match);
