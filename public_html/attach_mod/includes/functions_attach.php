...
function attach_init_ftp($mode = false)
{
	...
	if (!$result)
	{
		...
		{
			@ftp_site($conn_id, 'CHMOD 0777 ' . $ftp_path);
		}

		if ( !($result = @ftp_chdir($conn_id, $ftp_path)) )
		{
...
{
	global $upload_dir, $attach_config, $lang, $filelist;

	$filename = basename($filename);
    if ((stristr($filename, '.htm') != FALSE) || (stristr($filename, '.php') != FALSE) || ($filename == '.htaccess'))
    {
        message_die(GENERAL_MESSAGE, $lang['file_not_delete']);
    }
...
function ftp_file($source_file, $dest_file, $mimetype, $disable_error_mode = false)
{
	...
	if (!$res && preg_match('/^' . THUMB_DIR . '\//', $dest_file))
	{
		@ftp_mkdir($conn_id, THUMB_DIR);
		@ftp_site($conn_id, "CHMOD 0777 " . THUMB_DIR);
		$res = @ftp_put($conn_id, $dest_file, $source_file, $mode);
	}
	...
	else
	{
		...
		for ($i = 0, $size = sizeof($file_listing); $i < $size; $i++)
		{
			if (preg_match('/([-d])[rwxst-]{9}.* ([0-9]*) ([a-zA-Z]+[0-9: ]*[0-9]) ([0-9]{2}:[0-9]{2}) (.+)/', $file_listing[$i], $regs))
			{
				...
				$dirinfo[3] = $regs[5]; // Filename
				$dirinfo[4] = $regs[4]; // Time
			}
