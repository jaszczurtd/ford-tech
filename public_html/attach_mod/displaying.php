		...
		for ($i = 0, $size = sizeof($attachment_list); $i < $size; $i++)
		{
			...
			if (!$denied)
			{
				...
				if ($thumbnail)
				{
					...
					//
					$pic_size = @getimagesize('./' . $upload_dir . '/' . $attachment_list[$i]);
					$pic_width = ($pic_size[0] + 20);
					$pic_height = ($pic_size[1] + 25);

					$template->assign_block_vars('postrow.attach.cat_thumb_images', array(
						'DOWNLOAD_NAME' => $display_name,
						'IMG_SRC' => "javascript:displayWindow('$filename',$pic_width,$pic_height)",
	...
	for ($i = 0; $i < $num_attachments; $i++)
	{
		...
		if (!$denied || $userdata['user_level'] == ADMIN)
		{
			...
			if ($thumbnail)
			{
				...
				if (intval($attach_config['allow_ftp_upload']) && (@phpversion() >= '4.0.5') && preg_match('/(http|https)\:\/\//i', $attach_config['download_path']))
				{
					$pic_size = @getimagesize(trim($attach_config['download_path']) . '/' . $attachments['_' . $post_id][$i]['physical_filename']);
				}
				else
				{
					$pic_size = @getimagesize('./' . $upload_dir . '/' . $attachments['_' . $post_id][$i]['physical_filename']);
				}
				$pic_width = ($pic_size[0] + 20);
				$pic_height = ($pic_size[1] + 25);
